...
public class Node {
   ...
   * @param block the block
   */
  public void receiveBlock(final Block block) {
    if (this.consensusAlgo.isReceivedBlockValid(block, this.block)) {
      // czas weryfikacji
      final long blockVerificationTimeMs = Long.parseLong(System.getProperty("verifms", "59"));
      putTask(
          new simblock.task.Task() {
            @Override
            public long getInterval() {
              return blockVerificationTimeMs;
            }

            @Override
            public void run() {
              // po weryfikacji — akceptacja i propagacja bloku
              Node.this.acceptAndRelayBlock(block);
            }
          });
  ...
  // Nowa metoda pomocnicza w klasie Node — logika przeniesiona 1:1
  // z wnętrza dawnego run(); tutaj "this" oznacza już Node, więc
  // wszystkie pola i metody rozwiązują się poprawnie.
  private void acceptAndRelayBlock(final Block block) {
